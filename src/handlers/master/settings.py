...
def _guide_pages(*, plan_is_pro: bool) -> list[str]:
    ...
    return [
        (
            ...
            "<b>Переименование (алиасы)</b>\n"
            "Ты можешь переименовать Telegram‑клиента <b>для себя</b> — так, как тебе удобно.\n"
            "Это влияет только на то, как клиент отображается у тебя (списки/расписание/карточки).\n"
            "Сам клиент при этом своё имя не меняет.\n\n"
            "<b>Где:</b> «👥 Клиенты» → открыть клиента → «✏️ Редактировать клиента» → «✏️ Изменить имя».\n\n"
