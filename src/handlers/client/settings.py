...
def _guide_pages() -> list[str]:
    return [
        ...
        (
            "<b>2) Мои мастера и переименование</b>\n\n"
            "В «💇‍♀️ Мои мастера» можно:\n"
            "• открыть карточку мастера;\n"
            "• <b>переименовать мастера для себя</b> (кнопка «✏️ Переименовать»).\n\n"
            "Это влияет только на отображение имени у тебя (в списках/записях).\n"
            "Имя мастера для других клиентов не меняется."
        ),
        (
            "<b>3) Мои записи</b>\n\n"
        ...
        (
            "<b>6) Если что-то пошло не так</b>\n\n"
...
async def _dispatch_without_client(  # noqa: C901, PLR0911, PLR0912, PLR0915
    ...
    *,
    ...
    if action == "delete_confirm":
        ...
        async with active_session() as session:
            result = await DeleteClientPersonalData(session).execute(telegram_id=telegram_id)
