...
def _kb_invoice(*, invoice_url: str | None, invoice_id: int, contact: str) -> InlineKeyboardMarkup:
    ...
    if invoice_url:
        ...
        rows.append([InlineKeyboardButton(text=txt.btn_pay(), callback_data=_START_CB)])
    ...
    *,
    ...
    if not result.ok:
        ...
        if result.error == CreateProInvoiceError.ALREADY_PRO:
            await callback.bot.send_message(chat_id=callback.from_user.id, text=txt.pro_paid(), parse_mode="HTML")
            return
        await callback.bot.send_message(
            chat_id=callback.from_user.id,
            text=txt.pro_error(),
...
async def billing_pro_new_invoice(
    ...
    if express_pay_client is None:
        ...
        return
    await _create_and_show_invoice(callback, express_pay_client=express_pay_client, reuse_waiting=False)


@router.callback_query(UserRole(ActiveRole.MASTER), F.data.startswith(_CHECK_PREFIX))
async def billing_pro_check(  # noqa: C901
    callback: CallbackQuery,
    express_pay_client: ExpressPayClient | None = None,
) -> None:
