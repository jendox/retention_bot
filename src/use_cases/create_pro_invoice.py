...
class CreateProInvoiceError(StrEnum):
...
class CreateProInvoiceRequest:
    master_telegram_id: int
    amount: Decimal
    currency: CurrencyCode = CurrencyCode.BYN
    description: str = "Pro subscription"
...
class CreateProInvoiceResult:
    ...
    error: CreateProInvoiceError | None = None
    error_detail: str | None = None


class CreateProInvoice:
    ...
    @staticmethod
    def _error(*, error: CreateProInvoiceError, error_detail: str | None = None) -> CreateProInvoiceResult:
        return CreateProInvoiceResult(ok=False, error=error, error_detail=error_detail)

    async def _load_master(self, telegram_id: int):
        try:
            return await self._masters.get_by_telegram_id(telegram_id)
        except MasterNotFound:
            return None

    @staticmethod
    def _validate(request: CreateProInvoiceRequest) -> CreateProInvoiceResult | None:
        if request.master_telegram_id <= 0:
            return CreateProInvoice._error(
    ...
    @staticmethod
    def _compute_expires_at(request: CreateProInvoiceRequest, *, now_utc: datetime) -> datetime | None:
    ...
    async def execute(self, request: CreateProInvoiceRequest) -> CreateProInvoiceResult:
        invalid = self._validate(request)
        if invalid is not None:
            return invalid

        master = await self._load_master(request.master_telegram_id)
        if master is None:
            return self._error(error=CreateProInvoiceError.MASTER_NOT_FOUND)

        plan = await self._entitlements.get_plan(master_id=master.id)
        if plan.is_pro:
            return self._error(error=CreateProInvoiceError.ALREADY_PRO)

        now_utc = datetime.now(UTC)
        if request.reuse_waiting:
            existing = await self._maybe_reuse_waiting(master_id=master.id, now_utc=now_utc)
            if existing is not None:
                ev.info(
                    "billing.pro_invoice_reused",
                    master_id=master.id,
        ...
        try:
            ...
            ev.warning("billing.pro_invoice_provider_error", master_id=master.id, error=str(exc))
            return self._error(error=CreateProInvoiceError.INVALID_REQUEST, error_detail="provider_error")
        ...
        return CreateProInvoiceResult(ok=True, invoice=created, invoice_url=invoice_url, reused=False)
